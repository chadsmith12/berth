...
- Decisions are ask-or-flag, never silent: project (always, even a single one), environment (--env always wins; interactively, for an existing project only environments with a local `docker-compose.<env>.yml` are offered — one without a compose cannot launch — plus "create a new", which is validated against the compose files on disk and refuses with a "run berth generate --env <name>" error when missing; a project with nothing to list falls back to the production default; non-interactive always uses the production default), server (skipped when the team has exactly one), database (`none` accepted), db-name (strict: resource's name offered interactively, required non-interactively), deploy key, branch, domain. Non-interactive misses are usage errors naming the flag and listing options.
- Order matters: `launch` resolves project then environment *before* reading the compose file, because the environment names the compose file and its port. The session is opened with a preliminary app dir for placement; `sess.RepoRoot` is set to the final app dir once the environment is chosen.
- Domain: the port comes from the compose file — portless domains get it appended, explicit ports are cross-checked and refused on mismatch. The scheme is preserved and drives TLS: `https://` enables a Coolify certificate, `http://` does not; only http/https are accepted, a scheme-less host defaults to http.
...
- Healthchecks are owned by the compose, not Coolify (compose apps don't use the app Healthcheck page). Only web services (`app`, `ssr`) define a `healthcheck:`; every non-web service (`worker-N`, `horizon`, `scheduler`) sets `healthcheck: disable: true` **and** `exclude_from_hc: true` — the Coolify custom field that keeps them out of the stack's aggregated health (otherwise "no healthcheck" shows even when the app is healthy). `exclude_from_hc` is stripped by Coolify before `docker compose` sees it.
